...
<body>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $.ajax({
                type: 'GET',
                url: baseapiURL + 'GetUserPersonalInoAPI',
                contentType: "application/json",
                success: function(response) {
                    if (response) {
                        var jsonObj = JSON.parse(response);
                        $('#InputFullNameAccountDetails').val(jsonObj.personalInfo.firstName);
                        $('#InputLastNameAccountDetails').val(jsonObj.personalInfo.lastName);
                        //$('#InputCountryAccountDetails').val(jsonObj.personalInfo.nationality);

                        var dob = jsonObj.personalInfo.dob;
                        dob = moment(dob).format('DD/MM/YYYY');
                        $('#InputDobAccountDetails').val(dob);
                    } else {
                        showFailureDialogue("The service is temporarily unavailable, countries are not uploaded properly")
                        return;
                    }
            ...
            $.ajax({
                type: 'GET',
                url: baseapiURL + 'GetUserContactInfoAPI',
                contentType: "application/json",
                success: function(response) {
                    if (response) {
                        var jsonObj = JSON.parse(response);
        ...
        function accountdetailsValidation(mymessage) {
            ...
            fullnameerrorMessage = "Please Enter The Full Name without any special character";
            lastnameerrorMessage = "Please Enter The Last Name without any special character";
            nationalityerrorMessage = "Please Select The Nationality";
            doberrorMessage = "Please Select The Date OF Birth without any special character and in the format DD/MM/YYYY or DD-MM-YYYY";
            ...
            var regexFodDOB = /^(0?[1-9]|[12][0-9]|3[01])[\/\-](0?[1-9]|1[012])[\/\-]\d{4}$/;
            ...
            } else if (LastName == "" || LastName == 0 || regexSpecialCharsAndNum.test(LastName)) {
                showFailureDialogue(lastnameerrorMessage);
            } else if (dob == "" || dob == 0 || regexFodDOB.test(dob)) {
