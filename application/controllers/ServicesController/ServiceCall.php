...
class ServiceCall extends CI_Controller
{
    ...
    public function getprojectsbyuseridAPI()
    {
        try {
            $this->load->model('ServiceModel');
            $get_response = $this->ServiceModel->API($this->url->GET, $this->url->getprojectsbyuseridURL . "5f79d569681ba90024b35f9e", null, false);
            $response = json_decode($get_response[1]);
            if ($get_response[0] == $this->statuscodes->StatusCodeOK || $get_response[0] == $this->statuscodes->StatusCodeCreated) {
                echo json_decode(json_encode($get_response[1]), true);
            } else {
    ...
    public function addProjectAPI()
    {
        ...
        for ($i = 0; $i < $total_count; $i++) {
            ...
            $this->ProjectImagesPath->setImagePath($fileNewName);
            ${"newArray$fileName"} = array("imagePath" => $this->constant->ImagePathKey = $this->ProjectImagesPath->getImagePath());
    ...
    public function getAreaTypesByProjectIdAPI()
    {
        try {
            $json = file_get_contents('php://input');
            $obj = json_decode($json);
            $projectId = $obj->{"projectId"};
            $this->load->model('ServiceModel');
            $get_response = $this->ServiceModel->API($this->url->GET, $this->url->getareatypesbyprojectidURL($projectId), null, false);
            $response = json_decode($get_response[1]);
            if ($get_response[0] == $this->statuscodes->StatusCodeOK || $get_response[0] == $this->statuscodes->StatusCodeCreated) {
                echo json_decode(json_encode($get_response[1]), true);
            } else {
    ...
    public function getboqbyprojectidAPI()
    {
        try {
            $json = file_get_contents('php://input');
            $obj = json_decode($json);
            $projectId = $obj->{"projectId"};
            $this->load->model('ServiceModel');
            $get_response = $this->ServiceModel->API($this->url->GET, $this->url->getboqbyprojectidURL . $projectId, null, true);
            $response = json_decode($get_response[1]);
            if ($get_response[0] == $this->statuscodes->StatusCodeOK || $get_response[0] == $this->statuscodes->StatusCodeCreated) {
                echo json_decode(json_encode($get_response[1]), true);
            } else {
    ...
    public function getallboqsbyprojectidAPI()
    {
        try {
            $json = file_get_contents('php://input');
            $obj = json_decode($json);
            $projectId = $obj->{"projectId"};
            $this->load->model('ServiceModel');
            $get_response = $this->ServiceModel->API($this->url->GET, $this->url->getboqbyprojectidURL . "5f91a08aa189f600248cbcc3", null, true);
            $response = json_decode($get_response[1]);
            if ($get_response[0] == $this->statuscodes->StatusCodeOK || $get_response[0] == $this->statuscodes->StatusCodeCreated) {
                echo json_decode(json_encode($get_response[1]), true);
            } else {
    ...
    public function editProjectAPI()
    {
        ...
        $country = $_POST['InputWorkStationCountryName'];
        $city = $_POST['InputWorkStationCityName'];
        $user__id = $userid;
        $rating = $_POST['rating'];
      ...
      /*   $this->ProjectInterest->setProjectInterest('5f970f0cbe96120024ee61a6');
        ...
        array_push($projectInterestData, $test); */
    ...
    public function RedirecttoWorkstation()
    {
    ...
    public function getProjectsPerUserAPI()
    {
        $json = file_get_contents('php://input');
        $obj = json_decode($json);
        $user = '';
        ...
        $get_response = $this->api->callAPI($this->url->GET, $this->url->getBuyerProjectsByUserId . "/" . $__id, "", true);
        if ($get_response[0] == $this->statuscodes->StatusCodeOK || $get_response[0] == $this->statuscodes->StatusCodeCreated) {
            echo json_decode(json_encode($get_response[1]));
        } else {
            echo json_encode($get_response[0]);
    ...
    public function GetUserPersonalInoAPI()
    {       
        $get_response = $this->api->callAPI($this->url->GET, $this->url->getUserPersonalIno, "", true);
        if ($get_response[0] == $this->statuscodes->StatusCodeOK || $get_response[0] == $this->statuscodes->StatusCodeCreated) {
            echo json_decode(json_encode($get_response[1]));
        } else {
            echo json_encode($get_response[0]);
    ...
    public function GetUserContactInfoAPI()
    {
        $get_response = $this->api->callAPI($this->url->GET, $this->url->getUserContactInfo, "", true);
        if ($get_response[0] == $this->statuscodes->StatusCodeOK || $get_response[0] == $this->statuscodes->StatusCodeCreated) {
            echo json_decode(json_encode($get_response[1]));
        } else {
            echo json_encode($get_response[0]);
